...
//                                         0, NULL},		
    // Characteristic Declaration
    [QPPS_IDX_TX_VAL_CHAR]             =   {{ATT_UUID_16_LEN, (uint8_t *)"\x03\x28"}, PERM(RD, ENABLE), sizeof(qpps_value_char),
                                         sizeof(qpps_value_char), (uint8_t *)&qpps_value_char},
    // Characteristic Value
    [QPPS_IDX_TX_VAL]                  =   {{ATT_UUID_128_LEN, (uint8_t *)QPPS_TX_CHAR_UUID}, PERM(NTF, ENABLE), QPP_DATA_MAX_LEN,
                                         0, NULL},
    // Client Characteristic Configuration Descriptor
    [QPPS_IDX_TX_VAL_USER_DESP]          =   {{ATT_UUID_16_LEN, (uint8_t *)"\x01\x29"}, PERM(RD, ENABLE), sizeof("Send"),
                                         sizeof("Send"), (uint8_t *)"Send"},

		// Client Characteristic Configuration Descriptor
    [QPPS_IDX_TX_VAL_NTF_CFG]          =   {{ATT_UUID_16_LEN, (uint8_t *)"\x02\x29"}, PERM(RD, ENABLE)|PERM(WR, ENABLE), sizeof(uint16_t),
                                         0, NULL},
};
